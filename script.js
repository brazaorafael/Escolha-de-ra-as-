...
function card(r){
  ...
  // Tenta carregar a primeira URL que responder com sucesso
  const loadImage = (url) => new Promise((res, rej) => {
    if (url.includes("dog.ceo/api")) {
      // fetch dog.ceo JSON then resolve actual image URL
      fetch(url)
        .then(resp => resp.json())
        .then(js => js.status==="success" ? res(js.message) : rej(url))
        .catch(() => rej(url));
    } else {
      const testImg = new Image();
      testImg.src = url;
      testImg.onload = () => res(url);
      testImg.onerror = () => rej(url);
    }
  });
