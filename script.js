...
// **Atenção aqui**: carregamos o JSON validado
...
function card(r){
  // aqui usamos o array valid_images (não images)
  const imgs = r.valid_images.length? r.valid_images : r.images;
  const candidatos = [...imgs];

  // Dog CEO fallback
  const slug = r.slug.split("/").pop().toLowerCase();
  candidatos.push(`https://dog.ceo/api/breed/${slug}/images/random`);
  // placeholder final
  ...
  img.onerror = async function(){
    let idx = +this.dataset.idx + 1;
    if(idx < candidatos.length){
      const url = candidatos[idx];
      if(url.includes("dog.ceo/api")){
        try{
          const res = await fetch(url);
          const js = await res.json();
          this.src = js.message || placeholder;
        }catch{
