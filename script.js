/* ------------------------------------------------------------------
   Guia Inteligente de Raças – Lógica principal (atualização de fallback)
------------------------------------------------------------------ */
...
function card(r){
  // Prioriza foto principal, depois imagens extras
  const candidates = [];
  if(r.foto) candidates.push(r.foto);
  if(Array.isArray(r.images)) candidates.push(...r.images);

  // Dog CEO API fallback
  const slug = (r.slug || r.nome).split("/").pop().toLowerCase();
  ...
  img.alt = r.nome;
  img.dataset.idx = "0";
  img.src = candidates[0] || placeholder;
